...
{
  echo "/**"
  echo " * \`public\` 스키마의 제약 — 이름과, 무엇을 검사하는지."
  echo " *"
  echo " * 마이그레이션이 만든 스키마에서 supabase/tests/run.sh가 생성한다."
  echo " * 고치지 말 것 — \`pnpm test:db\`로 갱신한다."
  echo " *"
  echo " * \`errorMessage\`가 여기 모든 이름을 덮어야 하고 errorMessage.test.ts가 그것을 검사한다."
  echo " * 정의 쪽은 *값*을 싣는다 — 폼이 몇몇 상한을 미리 비추므로 사진 다섯 장을 올리기 전에"
  echo " * 위반이 잡히고, limits.test.ts가 그 거울이 여전히 맞는지 본다. 이름만으로는 값이"
  echo " * 어긋나는 것을 잡지 못한다 — \`items_price_max\`가 이름은 그대로인 채 값만 어긋났다."
  echo " */"
  ...
  echo "] as const"
  echo
  # Through jsonb rather than string-concatenated quotes: a CHECK body is full of
  # single quotes (`= ANY (ARRAY['spring'::text, …])`) and would need escaping
  # that is easy to get almost right. jsonb also sorts its keys deterministically,
  # which is what keeps the drift gate below from firing on reordering alone.
  echo "export const DB_CONSTRAINT_DEFS ="
  docker exec "$CONTAINER" psql -U postgres -tAq -c \
    "select jsonb_pretty(jsonb_object_agg(conname, pg_get_constraintdef(c.oid)))
