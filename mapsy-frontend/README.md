...
`staleTime`에서 TTL의 몇 분을 빼느냐가 그대로 **화면에 넘어가는 URL의 잔여 수명 바닥값**이다.
타이머로 재서명하는 게 없어서(마운트·포커스·재연결만 트리거고 셋 다 stale 판정을 거친다) stale
직전에 꺼내 쓴 항목은 정확히 그 여유분만 남은 URL을 들고 있다. 지금은 한 시간이고, 그 한 시간
동안 앱을 백그라운드로 안 보낸 채 계속 보고 있으면 사진이 깨진다. `gcTime`으로는 이 바닥을 못
올린다 — 두 값은 각각 *서명 시각*과 *마지막 관측자가 떠난 시각*부터 재는 다른 시계다.
